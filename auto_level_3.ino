...
// Modifié par Eric Célérault en Avril 2016.https://github.com/ericetmarine/Table-XY
 ...
 * Il est associé avec le programme Auto_level_2.0.py pour verifier la planeité du support/
...
// broche Laser
pinMode(coolant,OUTPUT);   // met broche en sortie
digitalWrite(coolant, LOW);// met broche niveau BAS
...
void help() {
	...
	Serial.println(F("Par X.HINAULT / E.CELERAULT- www.mon-club-elec.fr - GPLv3"));
	...
	Serial.println(F("G28 [X] [Y] [Z]; - home axis"));
        Serial.println(F("G54 Recherche Auto_Level"));
	Serial.println(F("G90; - absolute mode"));
	Serial.println(F("G91; - relative mode"));
	Serial.println(F("G92 [X(steps)] [Y(steps)]; - change logical position"));
	Serial.println(F("M8; - Marche Laser"));
        Serial.println(F("M9; - Arret Laser"));
        Serial.println(F("M18; - disable motors"));
	Serial.println(F("M100; - Ce Message"));
	Serial.println(F("M114; - positions et vitesse"));
...
void analyseG() {
	...
	case 54: // detection fin de course axe Z
	  //Serial.println(F("recherche fin de course axe Z"));
	  G54();
	  sendOK();
	  break;
...
void analyseM() {
  ...
  switch(cmd) {
        ...
        case 8://activation Laser
          Serial.println(F("M8"));
          laser_mode=1;
          digitalWrite(coolant, HIGH);
          Serial.println(laser_mode?"Laser On":"Laser Off");
          sendOK();
          break;
          
          case 9://désactivation Laser
          Serial.println(F("M9"));
          laser_mode=0;
...
//-------G54----- recherche fin de course Axe Z sans RAZ de l'axe

void G54()  {
float pas;
pas = 0;
Serial.println(F("position depart :"));
Serial.println(pz);
...
onestep(2,-1);//un pas en arriere

tick();// 	delay de feedrate

if (digitalRead(pin_ENDSTOP[2])==APPUI)  {// teste fin course

//Serial.println("Contact fin de course trouve");

delay(100);//anti-rebond

trouve=false;//pour sortir du While
...
pzn=pas/2560;
pz=pz-pzn;
//Serial.println(F("Decalage :"));
//Serial.println(pas);
//Serial.println(pzn);
//Serial.println(pz);
where_2();

} // fin recherche fin de course
